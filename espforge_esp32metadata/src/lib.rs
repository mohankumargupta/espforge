...
impl BoardDatabase {
    pub fn load() -> Self {
        ...
        let chips: Chips = toml::from_str(chips_data)
            .expect("Failed to parse chips.toml");
        
        // Load boards.toml
        let boards_data = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/data/boards.toml"));
        let boards: Boards = toml::from_str(boards_data)
            .expect("Failed to parse boards.toml");
