...
pub fn generate_peripheral_registry(model: &EspforgeConfiguration) -> Result<TokenStream> {
    ...
    let mut init_logic: Vec<TokenStream> = vec![];
    let mut struct_init: Vec<TokenStream> = vec![];
    ...
    if model.is_embassy() {
        ...
        // NOTE: stack is NOT stored in PeripheralRegistry.
        // It is created in main() after wifi init and passed directly to Components::new().
    }
