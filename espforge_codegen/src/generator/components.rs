...
use anyhow::Result;
use espforge_common::{Component, EspforgeConfiguration};
...
pub fn generate_component_registry(model: &EspforgeConfiguration) -> Result<TokenStream> {
    ...
    for (name, component) in sorted_components {
        match component {
            Component::LED { gpio } => {
                component_builders::led::generate(
                    name,
                    gpio,
                    &mut fields,
                    &mut init_logic,
            ...
            Component::Button { gpio, pull_up } => {
                component_builders::button::generate(
                    name,
                    gpio,
                    *pull_up,
                    &mut fields,
            ...
            Component::SpiDevice { spi, cs } => {
                component_builders::spi_device::generate(
                    name,
                    spi,
                    cs.as_deref(),
                    &mut fields,
            ...
            Component::I2cDevice { i2c, address } => {
                component_builders::i2c_device::generate(
                    name,
                    i2c,
                    *address,
                    &mut fields,
            ...
            Component::UartDevice { uart, baud } => {
                component_builders::uart_device::generate(
                    name,
                    uart,
                    *baud,
                    model,
