...
fn emit_lib(ir: &DeviceTree, catalog: &espforge_model::driver::Registry) -> String {
    // Re-export every component/device type the project instantiates so that
    // `app.rs` (which may only `use crate::{...}`) can name them without
    // reaching into `espforge_runtime::components::`/`devices::` directly. The
    // type name comes from the driver (`type_name()`) and the namespace from
    // the instance tier — the same convention `emit_generated` uses for the
    // `Components`/`Devices` field types. This is derived per-project, so any
    // component (Led, Button, I2cBus, …) or device type becomes reachable via
    // `crate::` automatically.
    //
    // `Delay` is also re-exported: it is a runtime singleton accessed via
    // `ctx.delay`, but `app.rs` needs to name its *type* (e.g. for a spawned
    // embassy task's parameter). `Logger` is intentionally NOT re-exported —
    // app code only ever uses it through `ctx.logger`.
    ...
    // `component!`/`device!` macros live at the generated crate root so `app.rs`
    // can call `crate::component!(ctx, led)` (strict `crate::` import rule,
    // ADR-008). Kept as a plain string (not run through `format!`) so the
    // brace-heavy `macro_rules!` bodies are not mangled by format escaping.
