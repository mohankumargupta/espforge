...
impl ComponentProvisioner {
    fn validate(&self, components: &HashMap<String, Component>, esp32: &Esp32Config) -> Result<()> {
         for (name, spec) in components {
            let plugin = find_plugin(&spec.driver)
                .ok_or_else(|| anyhow::anyhow!("Unknown driver '{}' for component '{}'", spec.driver, name))?;

            // Plugin-specific validation
            plugin.validate(&spec.properties)
            ...
            for dep in deps {
                if dep.kind == DependencyKind::Pin || dep.kind == DependencyKind::Peripheral {
                     ...
                     if !Self::hardware_exists(esp32, res_name) {
                         bail!("Component '{}' references unknown hardware resource '{}'", name, res_name);
                     }
                }
            }
        }
    ...
    fn hardware_exists(esp32: &Esp32Config, name: &str) -> bool {
        esp32.gpio.contains_key(name) || esp32.spi.contains_key(name) ||
        esp32.i2c.contains_key(name) || esp32.uart.contains_key(name)
    }
}