...
impl CoreDependencies {
    pub fn add(doc: &mut DocumentMut, versions: &VersionResolver) -> Result<()> {
        ...
        let platform_dep = Self::create_dependency("espforge_platform", &platform_version, use_git, local_path.as_deref());
        let components_dep = Self::create_dependency("espforge_components", &components_version, use_git, local_path.as_deref());
        let devices_dep = Self::create_dependency("espforge_devices", &devices_version, use_git, local_path.as_deref());
    ...
    fn create_dependency(crate_name: &str, version: &str, use_git: bool, local_path: Option<&str>) -> Item {
